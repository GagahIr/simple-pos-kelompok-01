...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $categoryIds = collect(['Makanan', 'Minuman', 'Snack', 'Lainnya'])->map(fn(string $name) => Category::create(['name' => $name])->id)->all();
        $products = [];
        ...
        for ($t = 0; $t < 2500; $t++) {
            DB::transaction(function () use ($user, $productIds, $productPrices) {
                ...
                foreach ($details as $index => $detail) {
                    $details[$index]['transaction_id'] = $transactionId;
                    $details[$index]['created_at'] = now();
                    $details[$index]['updated_at'] = now();
                }
